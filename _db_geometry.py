...
class DamBreak2DGeometry(object):
    def __init__(self, container_width=4.0, container_height=3.0,
                 ...
                 ro=1000.0, co=1.0, with_obstacle=False,double_dam=False,
    ...
    def create_particles(self, nboundary_layers=2, nfluid_offset=2,
                         hdx=1.5, **kwargs):
        nfluid = self.nfluid
        xfl, yfl = self.get_fluid(nfluid_offset)
        if self.iisph:
            fluidl = get_particle_array_iisph(name='fluidl', x=xfl, y=yfl)
        else:
            fluidl = get_particle_array_wcsph(name='fluidl', x=xfl, y=yfl)

        fluidl.gid[:] = list(range(fluidl.get_number_of_particles()))

        np = nfluid

        if self.double_dam:
            xfr, yfr = self.get_fluid(nfluid_offset)
            xfr += (self.container_width - self.fluid_column_width)
            if self.iisph:
                fluidr = get_particle_array_iisph(name='fluidr', x=xfr, y=yfr)
            else:
                fluidr = get_particle_array_wcsph(name='fluidr', x=xfr, y=yfr)

            fluidr.gid[:] = list(range(fluidr.get_number_of_particles()))
        ...
        fluidl.h[:] = numpy.ones_like(xfl) * hdx * dx
        if nfluid_offset == 2:
            fluidl.m[:] = dx * dy * ro * 0.5
        else:
            fluidl.m[:] = dx * dy * ro
        fluidl.rho[:] = ro
        if not self.iisph:
        ...
        if self.double_dam:
            fluidr.h[:] = numpy.ones_like(xfr) * hdx * dx
            if nfluid_offset == 2:
                fluidr.m[:] = dx * dy * ro * 0.5
            else:
                fluidr.m[:] = dx * dy * ro
            fluidr.rho[:] = ro
            if not self.iisph:
        ...
        print("2D dam break with %d fluid, %d boundary particles"%(
            fluidl.get_number_of_particles(),
